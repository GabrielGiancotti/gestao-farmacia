...
namespace Gestao_Farmacia.Controllers
{
    ...
    public class AutenticacaoController : ControllerBase
    {
        ...
        /// <summary>
        /// Valida se o token externo informado é válido.
        /// </summary>
        /// <returns>IActionResult</returns>
        /// <response code="200">Caso o token seja validado com sucesso.</response>
        /// <response code="400">Caso tenha tido algum problema durante a validação do token externo.</response>
        /// <response code="401">Caso tenha tido algum problema de autenticação durante a validação do token externo.</response>
        /// <response code="500">Caso tenha tido algum problema interno no servidor durante a validação do token externo.</response>
        [HttpGet("externo/validar/token")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<Resposta<object>>> ValidarTokenExternoAsync()
        {
            try
            {
        ...
        /// <response code="401">Caso tenha tido algum problema de autenticação durante a validação do token.</response>
        /// <response code="500">Caso tenha tido algum problema interno no servidor durante a validação do token.</response>
